...
urlpatterns = [
    ...
    path('user/update/', UpdateUserView.as_view(), name='update-user'),             # 회원 정보 수정
    ...
    path('signup/', include('dj_rest_auth.registration.urls')),                     # 회원가입: dj_rest_auth의 회원가입 (URL 포함)
    path('logout/', LogoutView.as_view(), name='logout'),                           # 로그아웃 (JWT 토큰 제거)
]
